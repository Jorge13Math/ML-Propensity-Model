...
class Utils:
    ...
    def plot_variable(self, df, column, df_result=False):
        ...
        self.df_column = pd.DataFrame({f"{column}": eval(f"df.{column}").value_counts().index,
                                       'Count': eval(f"df.{column}").value_counts().values})
        if df_result:
    ...
    def plot_variable_per_target(self, df, column, target, df_result=False):
        ...
        self.df_class = pd.DataFrame({'0': eval(f"df[df[target]==0].{column}").value_counts(),
                                      '1': eval(f"df[df[target]==1].{column}").value_counts()},
                                     index=eval(f"df.{column}").value_counts().index)
        if df_result:
